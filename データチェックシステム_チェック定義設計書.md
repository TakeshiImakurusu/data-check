...
| DEKISPART_CHK_0053 | stdZip空白チェック | エラー |
...
- **説明**: stdFlg4がTRUE（敬称が様）であり、かつstdName（ユーザー名1）に法人名キーワード（株式、有限、合同、合資、合名）または個人名チェックリストのキーワードが含まれている場合に検出されます。#### DEKIS
PART_CHK_0011
...
- **説明**: stdSale1が「004359」の場合、stdSale2（販売店コード2）は「00r」で始まる必要があります。また、stdSale2が「00r」で始まる場合、stdSale1は「004359」である必要があります。この条件に合致しない場合に検出されます。

#### DEKISPART_CHK_0020
- **名称**: stdSale1「000286」とstdSale2の組み合わせチェック
- **重要度**: エラー
- **エラーメッセージ**: stdSale1が000286の場合、stdSale2はkeで始まる必要があります。
- **説明**: stdSale1が「000286」の場合、stdSale2（販売店コード2）は「ke」で始まる必要があります。また、stdSale2が「ke」で始まる場合、stdSale1は「000286」である必要があります。この条件に合致しない場合に検出されます。#
### DEKISPART_CHK_0021
...
- **説明**: stdTsel（担当者コード）に紐づく担当者マスタの担当者名の先頭が、「×」または「・」である場合に検出されます。これらは担当者名として不適切です。#### 
DEKISPART_CHK_0041
...
- **名称**: stdZip空白チェック
- **重要度**: エラー
- **エラーメッセージ**: stdZipは空白であってはなりません。
- **説明**: stdZipは必須項目です。空白の場合は正しい郵便番号を入力してください。
